...
{
    public class ViewMyOrdersViewModel : INotifyPropertyChanged
    {
        ...
        public ViewMyOrdersViewModel(User user)
        {
            _currentUser = user;
        ...
        private void ViewOrderDetails(OrderViewModel order)
        {
            ...
            MessageBox.Show($"Order Details for Order #{order.OrderId}\n\n" +
                           $"Status: {order.OrderState}\n" +
                           $"Date Created: {order.CreatedAt:dd/MM/yyyy HH:mm}\n" +
        ...
        private void CancelOrder(OrderViewModel order)
        {
                               ...
                               "Confirm Cancellation", MessageBoxButton.YesNo, MessageBoxImage.Warning) == MessageBoxResult.Yes)
            {
                try
                {
                    ...
                    {
                        connection.Open();
                        using (var command = new SqlCommand("UPDATE Orders SET OrderState = @OrderState WHERE Id = @OrderId AND UserId = @UserId", connection))
                        {
                            ...
                            {
                                MessageBox.Show($"Failed to cancel order. Order #{order.OrderId} not found or you don't have permission to cancel it.",
